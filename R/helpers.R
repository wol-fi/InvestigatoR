...
select_dates_by_offset <- function(dates, window_size, step_size, offset, rolling=TRUE) {
  ...
  date_intervals <- date_intervals |>
    dplyr::mutate(prediction_start=as.Date(
      ifelse(!is.na(dplyr::lag(prediction_end))&prediction_start>=dplyr::lag(prediction_end),dplyr::lag(prediction_end),prediction_start)
      , origin=.Date(0)))
